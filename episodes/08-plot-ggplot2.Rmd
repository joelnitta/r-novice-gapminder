...
```{r blank-ggplot, message=FALSE, fig.alt="何も描画されていない、ggplot() に美的マッピングを追加する前の空のプロット。"}
library("ggplot2")
...
```{r ggplot-with-aes, message=FALSE, fig.alt="X軸に gdpPercap、Y軸に lifeExp を表示する散布図のための軸だけがあるプロットエリア。"}
...
```{r lifeExp-vs-gdpPercap-scatter, message=FALSE, fig.alt="GDP と平均寿命の関係を示す散布図。データポイントが描画されている。"}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
  geom_point()
```
...
```{r ch1-sol, fig.cap="平均寿命が時間とともに増加していることを示す、年と平均寿命の散布図。"}
...
```{r ch3-sol, fig.alt="GDPと平均寿命の散布図で、変数間の関係を示すトレンドラインを追加したグラフ。データポイントが拡大され、オレンジ色で、透過性なしで表示されています。"}
...
```{r axis-scale, fig.cap="X軸が対数スケールでデータが拡散したGDPと平均寿命の散布図"}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
...
```{r lm-fit, fig.alt="GDPと平均寿命の散布図に、変数間の関係を要約する青いトレンドラインが追加されたグラフ。グレーの影は95%の信頼区間を示します。"}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
...
```{r lm-fit2, fig.alt="GDPと平均寿命の散布図に、少し太めの青いトレンドラインが追加されたグラフ。"}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
...
```{r ch4a-sol, fig.alt="GDPと平均寿命の散布図に、変数間の関係を要約するトレンドラインが追加されたグラフ。データポイントが拡大され、オレンジ色で、透過性なしで表示されています。"}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
